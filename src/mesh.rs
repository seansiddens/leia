...
// TODO: Use Affine3A instead of Mat4?
...
impl Mesh {
    ...
    pub fn translation(&mut self, translation: Vec3A) {
        ...
        self.model_to_world = Mat4::from_scale_rotation_translation(
            self.scale.into(),
            self.rotation.into(),
...
impl Hittable for Mesh {
    fn hit(&self, r: &Ray, t_min: f32, t_max: f32, rec: &mut HitPayload) -> bool {
        ...
        let ray = Ray::new(
            self.world_to_model
                .transform_point3(r.origin().into())
                .into(),
            self.world_to_model
                .transform_vector3(r.direction().normalize().into())
                .into(),
        );
        ...
        if use_bvh {
            let hit_anything = if self.bvh.hit(&ray, t_min, t_max, rec) {
                ...
                rec.world_position = self
                    .model_to_world
                    .transform_point3(rec.world_position.into())
                    .into();
                rec.world_normal = self
                    .model_to_world
                    .transform_vector3(rec.world_normal.into())
                    .into();
            ...
            // Transform the hit position and hit surface normal back to world space.
            rec.world_position = self.model_to_world.transform_point3(rec.world_position.into()).into();
            rec.world_normal = self.model_to_world.transform_vector3(rec.world_normal.into()).into();
